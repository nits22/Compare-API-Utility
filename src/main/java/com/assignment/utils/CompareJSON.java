...
public class CompareJSON {
	...
	public boolean compare(String res1, String res2) {
		...
		try {
			...
			loggerWrapper.myLogger.error("Incorrect JSON format - " + jpe.getStackTrace());
			try {
				if (res1.equals(res2))        // for handling when response is just a plain text
					return true;
			} catch (Exception e) {
				loggerWrapper.myLogger.error(loggerWrapper.getStackTrace(e.fillInStackTrace()));
				return false;
			}
		} catch (NullPointerException ne) {
			loggerWrapper.myLogger.error("Response is null " + ne.getStackTrace());
